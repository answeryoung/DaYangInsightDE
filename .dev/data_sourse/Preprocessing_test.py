...
output_json_name = 'ecg-data-test.json'
    ...
    for s in subjects:
        ...
        for m in subjects[s]['measurements']:
            ...
            for w in range(nWindow):
                ...
                dump_to_json( output_json_name, topic, record, w, subjects, 0, -nPeaks-1, sample)
                for p in range(nPeaks):
                    ...
                    dump_to_json( output_json_name, topic, record, w, subjects, idx, idx_neg, sample)
...
def dump_to_json( output_file_name, topic, record, window, subjects, idx, idx_neg, signal_segment):
    Odict = {}
    Odict['topic'] = topic
