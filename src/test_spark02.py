...
conf = SparkConf().setAppName("JRS").set("spark.streaming.backpressure.enable","true")
...
def main():
  ...
  # Create DataSet representing the stream of input lines from kafka
    ...
    sc = spark.sparkCon
sc  = SparkContext(conf=conf)                                                                                                                                                                                                          ssc = StreamingContext(sc, streaming_window)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: xxx.py <broker_list> <subscribe-type> <topics>", file=sys.stderr)
        sys.exit(-1)
