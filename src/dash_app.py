...
metadata = get_metadata(conn, topic, table_suffix)
...
app.layout = html.Div(
    children=[
        ...
        html.Div(
            html.H5('Measurement S/N: '+metadata['measurement_datetime'])
        ),
        html.Div(
            html.H5('Patient Age: '+str(int(metadata['patient_age'])))
        ),
        html.Div(
            [
...
def update_graph_scatter(unused_var):
    ...
    if time_s[-1] < time_s[0] + window_width_s + 2 and \
            idx0 < number_of_segments-1:
        
        idx0, time_s, signal, t_features = \
            get_data(idx0, df_batch_length, time_s, signal, t_features)

    data, time_s, signal, t_features, axes_lim = \
        get_data_go(num_points, num_shift_points, time_s, signal, t_features)
...
###############################################################################
def get_data(idx_lim, batch_length,
             time_input, signal_input, t_features_input):
    sql_get_latest_data = ("""
            SELECT
    ...
    time_cur = np.arange(
        time_0,
        time_0 + len(signal_cur) * time_step_orz, time_step_orz
    )
    ...
    t_features_cur = \
        data_df.loc[:, ['p_s', 'q_s', 'r_s', 's_s', 't_s']]\
        .to_numpy()
    
    if len(t_features_input) == 0:
        t_features_output = t_features_cur
    else:
        t_features_output = np.concatenate(
            (t_features_input, t_features_cur),
            axis=0
        )
...
def get_data_go(n_points, n_shift_points,
                time_s_input, signal_input, t_features_input):
    ...
    f0 = np.argwhere(t_features_input[:, 2] < X[0])
    if len(f0) == 0:
        f0 = 0
    else:
        f0 = f0.max()
    ...
    v_features_output = np.zeros_like(t_features_output)
    data = [signal_go]
    feature_names = ['P-peak', 'Q-peak', 'R-peak', 'S-peak', 'T-peak']
    for p in range(5):
        data.append(
            go.Scatter(
                x=list(t_features_output[:, p])
                , y=list(v_features_output[:, p])
